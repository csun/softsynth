...
DelayProcessor::DelayProcessor() {
  addParameter(isOn = new AudioParameterBool("isOn", "On", false));
  addParameter(time = new AudioParameterFloat("time", "Time (ms)", 0.0f,
                                              1000.0f, 200.0f));
  addParameter(feedback = new AudioParameterFloat("feedback", "Feedback", 0.0f,
                                                  1.0f, 0.25f));
  addParameter(mix = new AudioParameterFloat("mix", "Mix", 0.0f, 1.0f, 0.5f));
}
...
void DelayProcessor::processSample(float &sample) {
  ...
  delayLine.pop_front();
  delayLine.push_back(in + (delaySample * feedbackAmt));

  sample = std::min(
      1.0f, std::max(0.0f, (in * (1.0f - mixAmt)) + (delaySample * mixAmt)));
}

void DelayProcessor::resizeDelayLine() {
  int desiredSize = std::ceil((time->get() / 1000) * currentSampleRate);
